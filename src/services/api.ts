...
export type CleaningRowsPageParams<
...
export type CleaningRowsPageResult<
  T extends Record<string, any> = Record<string, any>,
> = {
  rows: T[];
  total: number;
...
export type CorrectionManualChange = {
  targetField: string;
  previousSourceField: string | null;
  updatedSourceField: string | null;
...
export const API_BASE_URL =
  ...
  "https://fraser-calls-father-syndication.trycloudflare.com";
...
function applyAiFixToIssue(
  rows: any[],
  issue: DataIssueGroup,
): { rows: any[]; fixedCount: number } {
  const nextRows = [...rows];
  let fixedCount = 0;
  ...
  issue.rows.forEach((rowIndex, issueRowPosition) => {
    ...
    if (issue.issue_type === "invalid_email") {
      ...
      if (existing && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(existing)) {
        const sanitized = existing.includes("@")
          ? existing
          : `${existing}@example.com`;
        nextRows[rowIndex] = {
          ...row,
          [issue.column]: sanitized.replaceAll(" ", ""),
        };
        fixedCount += 1;
      }
      return;
...
export const api = {
  ...
  aiFixIssue: async (
    data: any[],
    issue: DataIssueGroup,
  ): Promise<{ data: any[]; fixedCount: number }> => {
    await delay(900);
  ...
  submitSessionEdits: async (
    ...
    return apiRequest<SubmitEditsResponse>(
      `${API_ROUTES.editBySession}/${sid}`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
  ...
  getColumnProfile: async (sessionId: string, column: string): Promise<any> => {
    return apiRequest<any>(
      `${API_ROUTES.columnProfile}/${sessionId}/${column}`,
      { method: "GET" },
    );
  },
  ...
  editCells: async (
    sessionId: string,
    edits: SubmitEditsRequest,
  ): Promise<SubmitEditsResponse> => {
    return apiRequest<SubmitEditsResponse>(
      `${API_ROUTES.editBySession}/${sessionId}`,
      {
        method: "POST",
  ...
  exportCleanedData: async (sessionId: string): Promise<Blob> => {
    const response = await fetch(
      `${API_BASE_URL}${API_ROUTES.export}/${sessionId}`,
      { method: "GET" },
    );
    if (!response.ok) {
  ...
  getSessionStatus: async (sessionId: string): Promise<{ stage: string }> => {
    const sid = encodeURIComponent(sessionId);
    return apiRequest<{ stage: string }>(`${API_ROUTES.sessionStatus}/${sid}`, {
      method: "GET",
    });
  },
  ...
  addressCorrector: async (sessionId: string): Promise<any> => {
    const sid = encodeURIComponent(sessionId);
    return apiRequest<any>(`${API_ROUTES.addressCorrector}/${sid}`, {
      method: "POST",
    });
  },
  ...
  previewCleaned: async (sessionId: string): Promise<any> => {
    const sid = encodeURIComponent(sessionId);
    return apiRequest<any>(`${API_ROUTES.previewCleaned}/${sid}`, {
      method: "GET",
    });
  },
  ...
  fetchSummaryData: async (sessionId: string): Promise<any> => {
    const sid = encodeURIComponent(sessionId);
    return apiRequest<any>(`${API_ROUTES.sessionSummary}/${sid}`, {
      method: "GET",
    });
  },
  ...
  refreshedData: async (sessionId: string): Promise<RefreshedDataResponse> => {
    const sid = encodeURIComponent(sessionId);
    return apiRequest<RefreshedDataResponse>(
      `${API_ROUTES.refreshedData}/${sid}`,
      { method: "GET" },
    );
  },
};
